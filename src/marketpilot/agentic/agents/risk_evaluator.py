...
def risk_evaluator_agent(
    state: RiskEvaluatorState, config: AgentConfig, tools: Dict[str, ToolCallable]
) -> RiskEvaluatorState:
    state.get("logs", []).append("Risk evaluator executing")
    ...
    state.setdefault("data", {}).setdefault("risk_assessment", []).append(
        "risk assessment did some analyses stuff"
    )
