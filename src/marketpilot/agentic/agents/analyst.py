...
def analyst_agent(
    state: AnalystState, config: AgentConfig, tools: Dict[str, ToolCallable]
) -> AnalystState:
    state.get("logs").append("Analyst executing")
    ...
    state.setdefault("data", {}).setdefault("analyst", []).append(
        "analyst did some analyse stuff"
    )
