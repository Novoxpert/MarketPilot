    ...
    def _import_tools(self) -> None:
        for tool_name, _cfg in self.tool_configs.items():
            ...
            if callable(fn):
                ...
                print(
                    f"[WARN] Tool function `{tool_name}_tool` not found in {module_path}"
                )
    ...
    def _import_agents(self) -> None:
        for agent_name, cfg in self.agent_configs.items():
            ...
            if callable(fn):
                typed_raw = cast(
                    Callable[
                        [AgentState, AgentConfig, Dict[str, ToolCallable]], AgentState
                    ],
                    fn,
                )
                ...
                print(
                    f"[WARN] Agent function `{cfg.name}_agent` missing in {role_module}"
                )
    ...
    def _import_conditions(self) -> None:
        for condition_name, _cfg in self.condition_configs.items():
            ...
            if callable(fn):
                ...
                print(
                    f"[WARN] Condition function `{condition_name}_condition` not found in {module_path}"
                )
